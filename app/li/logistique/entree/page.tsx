...
import RgpdNoticeLI from "@/app/components/RgpdLI";
    ...
    <main className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {phase === "choix" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          {[
            { id: "dechargement", img: "/dechargement.png", label: "Sunkvežimio pakrovimas/iškrovimas" },
            { id: "depotage", img: "/granule.jpg", label: "Granuliuotas iškrovimas" },
            { id: "quai", img: "/quai.jpg", label: "Krantinio krovimo iškrovimas" },
            { id: "expedition", img: "/expedition.png", label: "Cheminių medžiagų/pavojingų atliekų iškrovimas/gabenimas" },
          ].map(r => (
      ...
      {phase === "form" && (
        <form onSubmit={handleSubmit} className="w-[420px] space-y-4 mt-30">
          ...
          <input name="firstName" placeholder="Vardas" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="lastName" placeholder="Pavardė" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="company" placeholder="Įmonė" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="tractorRegistration" placeholder="Traktoriaus registracija" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="trailerRegistration" placeholder="Priekabos registracija" onChange={handleChange} className="border p-2 w-full text-black" required />
          <RgpdNoticeLI></RgpdNoticeLI>
