    ...
    <main className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {phase === "choix" && (
        <div className="flex flex-col items-center w-full max-w-4xl mt-12">
          ...
          <h2 className="text-2xl font-bold mb-8 text-center text-black uppercase tracking-wide">
            Escolha o motivo da sua visita
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            {[
              { id: "dechargement", img: "/dechargement.png", label: "Carga/descarga de caminhão" },
              { id: "depotage", img: "/granule.jpg", label: "Descarga granulada" },
              { id: "quai", img: "/quai.jpg", label: "Descarga no cais" },
              { id: "expedition", img: "/expedition.png", label: "Descarga/transporte de produtos químicos/resíduos perigosos" },
            ].map(r => (
      ...
      {phase === "form" && (
        <form onSubmit={handleSubmit} className="w-[420px] space-y-4 mt-30">
          ...
          <input name="firstName" placeholder="Nome próprio" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="lastName" placeholder="Nome" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="company" placeholder="Empresa" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="tractorRegistration" placeholder="Registo do trator" onChange={handleChange} className="border p-2 w-full text-black" required />
          <input name="trailerRegistration" placeholder="Registo de reboque" onChange={handleChange} className="border p-2 w-full text-black" required />
      ...
      {phase === "success" && (
        <div className="text-green-600 text-xl font-bold mt-4">Registado com sucesso!</div>
      )}
    </main>
  );
}